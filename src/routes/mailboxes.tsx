      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="basics" className="mt-4">
          ...
          <TabsContent value="smtp" className="grid grid-cols-2 gap-3 pt-3">
            ...
            <div><Label>Port</Label><Input type="number" min={1} max={65535} value={form.smtp_port} onChange={(e) => setForm({ ...form, smtp_port: Number(e.target.value) })} onBlur={(e) => { const v = Math.max(1, Math.min(65535, Math.floor(Number(e.target.value) || 0))); setForm(f => ({ ...f, smtp_port: v })); }} /></div>
