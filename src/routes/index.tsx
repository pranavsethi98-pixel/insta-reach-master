...
import {
  ArrowRight, Flame, Shield, CheckCircle2, X, Globe, Webhook, Calendar,
  Users, Quote, Download, Lock, Zap, Target, Inbox, Bot, Plus, Minus, Clock,
  Star,
} from "lucide-react";
...
import {
  MailboxRotationVisual, SequenceVisual, InboxVisual,
  PipelineVisual, AnalyticsVisual, AIComposeVisual,
...
function Landing() {
  ...
  return (
    <MarketingLayout>
      {/* ─── HERO ─── */}
      <section className="relative max-w-6xl mx-auto px-6 pt-16 md:pt-24 pb-20 overflow-hidden">
        {/* Spotlight glow */}
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center relative">
          ...
          <div className="lg:col-span-7">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/25 text-[11px] font-mono uppercase tracking-widest mb-7">
              <span className="relative flex w-1.5 h-1.5">
                <span className="absolute inset-0 rounded-full bg-success animate-ping opacity-75" />
                <span className="relative w-1.5 h-1.5 rounded-full bg-success" />
              </span>
              <span className="text-primary/80">v2.0 · Public beta · 12M+ sends / mo</span>
            </div>
            ...
            <p className="mt-7 text-lg md:text-xl text-muted-foreground max-w-xl leading-relaxed">
              Unlimited inboxes. Automatic warmup. AI sequences. One unified inbox for every reply. Everything your outbound team needs — in a single, clean workspace.
            </p>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              ...
              <Button size="lg" variant="outline" className="rounded-full text-base h-12 px-7 border-2" asChild>
      ...
      {/* ─── SOCIAL PROOF BAR ─── */}
      <section className="border-y border-border/60 py-5 bg-card/40 backdrop-blur">
        <div className="max-w-5xl mx-auto px-6 proof-bar-fade">
          <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10 text-sm">
            {[
              { n: "1,000+", l: "teams using EmailSend" },
              { n: "12M+", l: "emails sent / month" },
              { n: "99.2%", l: "inbox placement rate" },
              { n: "38%", l: "average reply rate" },
              { n: "< 5 min", l: "to first send" },
            ].map((s, i) => (
              <span key={i} className="flex items-center gap-1.5 text-muted-foreground whitespace-nowrap">
                <span className="text-foreground font-bold">{s.n}</span>
                {s.l}
              </span>
            ))}
          </div>
      ...
      <section className="relative max-w-5xl mx-auto px-6 pt-24 pb-12">
        <div className="text-eyebrow mb-6 text-center">Why we built this</div>
        <p className="text-center text-2xl md:text-4xl font-extrabold tracking-tight leading-[1.15] max-w-4xl mx-auto">
          We don't believe in{" "}
          <span className="text-muted-foreground/50 line-through decoration-destructive/50">credit packs</span>,{" "}
          <span className="text-muted-foreground/50 line-through decoration-destructive/50">per-mailbox tax</span>, or{" "}
          <span className="text-muted-foreground/50 line-through decoration-destructive/50">warmup that quietly burns your domain</span>.
      ...
      <section className="max-w-5xl mx-auto px-6 pt-8 pb-20">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="mt-4 text-muted-foreground text-lg">
            Per-mailbox pricing. Hidden warmup limits. Reply detection that misses half your wins. We rebuilt it.
          </p>
        </div>
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { p: "Per-inbox pricing", s: "Unlimited inboxes — every plan, every tier. No per-mailbox tax. Ever." },
            { p: "Warmup that hurts you", s: "Conversation-style warmup tuned for real engagement signals." },
            { p: "Replies in 8 dashboards", s: "One unified inbox. Every account. Sorted by intent so positives rise first." },
          ].map((x) => (
            <div key={x.p} className="bg-card border border-border rounded-2xl p-6 hover:border-primary/40 transition-colors">
              ...
              <div className="flex items-start gap-2">
                ...
                <span className="font-semibold text-sm leading-relaxed">{x.s}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="max-w-3xl mx-auto px-6 text-center mb-16">
        ...
        <p className="mt-4 text-muted-foreground text-lg">From the first cold email to the booked meeting — no duct tape, no missing pieces.</p>
      </section>

      {/* ─── FEATURE BLOCKS ─── */}
      <FeatureBlock
        eyebrow="Sending infrastructure"
        title="Unlimited inboxes. Smart rotation."
        ...
        bullets={["Connect a mailbox in 30 seconds", "Per-inbox daily caps + sending windows", "Randomized delays for natural patterns", "Auto-pause on bounce spikes"]}
      ...
      <FeatureBlock
        reverse
        eyebrow="AI Copilot"
        title="A campaign in 30 seconds."
        desc="Describe your ICP and offer. The Copilot writes the full sequence — subject lines, openers, follow-ups, breakup emails — in your voice. Launch-ready."
      ...
      <FeatureBlock
        eyebrow="Sequences"
        title="Multi-step flows that convert."
        ...
        bullets={["Drag-and-drop step builder", "Conditional branching on intent signals", "Spintax + dynamic variables", "A/B test subjects and bodies"]}
      ...
      <FeatureBlock
        ...
        title="Every reply, in one feed."
        desc="Stop juggling 8 inboxes. Every reply across every mailbox in a single place — sorted by intent so positives bubble to the top automatically."
        bullets={["Cross-mailbox unified inbox", "Auto-categorized by intent", "Reply from any connected sender", "Out-of-office detection"]}
        visual={<InboxVisual />}
      />
      ...
      <FeatureBlock
        ...
        desc="Positive replies auto-create deals. Drag through stages, log notes, forecast revenue. The CRM that lives where your pipeline is actually born."
        bullets={["Auto-create deals from positive replies", "Custom stages and fields", "Forecast by stage value", "Notes, tasks, and reminders"]}
      ...
      <FeatureBlock
        ...
        desc="Sends, opens, clicks, replies, and meetings — sliced by mailbox, campaign, and step. Spot what to scale and what to cut."
        bullets={["Real-time dashboards", "Per-step performance data", "Cohort and funnel views", "Export to CSV or push via webhook"]}
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="mt-3 text-muted-foreground text-lg">Every feature a serious outbound team needs — included.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { icon: Flame, title: "Free warmup network", desc: "Conversation-style warmup that builds and protects sender reputation across your entire domain fleet." },
            { icon: Shield, title: "Deliverability suite", desc: "SPF/DKIM/DMARC checks, spam-word linter, bounce management, and suppression lists — built in." },
            { icon: Calendar, title: "Native meeting scheduler", desc: "Book straight into Google or Outlook calendars without leaving the reply thread." },
            { icon: Globe, title: "Visitor identification", desc: "Know which prospects visit your site after receiving an email — with reverse-IP enrichment." },
            { icon: Webhook, title: "Webhooks & REST API", desc: "Push events to your CRM, Slack, or anywhere. Full API access included on Scale and above." },
            { icon: Users, title: "Team workspaces", desc: "Roles, shared campaigns, per-user mailbox assignment, and sub-account management for agencies." },
          ].map(({ icon: Icon, title, desc }) => (
            <div key={title} className="group bg-card border border-border rounded-2xl p-6 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_oklch(0.55_0.22_263/0.3)] transition-all duration-300">
      ...
      <section className="max-w-5xl mx-auto px-6 pb-24">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight">
            ...
            <span className="text-muted-foreground line-through decoration-destructive/60">Instantly</span> &{" "}
            <span className="text-muted-foreground line-through decoration-destructive/60">Smartlead</span>.
          </h2>
        </div>
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          ...
          {[
            ...
            <div key={i} className={`grid grid-cols-4 text-sm border-b border-border/60 last:border-0 ${i % 2 ? "bg-muted/10" : ""}`}>
              <div className="p-4 font-medium text-sm">{r.f}</div>
              <Cell on={r.e} primary />
              <Cell on={r.i} />
              <Cell on={r.s} />
            </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="text-center max-w-2xl mx-auto mb-12">
          <div className="text-xs font-bold uppercase tracking-widest text-primary mb-3">Customer stories</div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {[
            ...
            { q: "The warmup is the real deal. We're sending 5x more volume without a single placement issue. Worth it for that alone.", n: "Marcus Webb", r: "Founder, Quanta" },
            { q: "Setup was 10 minutes. The AI Copilot writes better subject lines than my team does. This is genuinely the future of outbound.", n: "Priya Patel", r: "VP Sales, Lumen" },
          ].map((t, i) => (
            <div key={i} className="bg-card border border-border rounded-2xl p-6 flex flex-col hover:border-primary/30 transition-colors">
              ...
              <p className="text-sm leading-relaxed flex-1 text-foreground/90">"{t.q}"</p>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-24">
        <div className="relative rounded-3xl bg-primary p-12 md:p-16 text-primary-foreground overflow-hidden text-center">
          {/* Dot pattern */}
          <div className="absolute inset-0 opacity-[0.08]" style={{ backgroundImage: "radial-gradient(circle at center, white 1px, transparent 1px)", backgroundSize: "28px 28px" }} />
          {/* Top glow */}
          <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-64 h-64 rounded-full bg-white/20 blur-3xl" />
          <div className="relative">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 text-xs font-mono uppercase tracking-widest mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" /> No credit card required
            </div>
            <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight">Start filling your pipeline today.</h2>
            <p className="mt-4 text-lg opacity-90 max-w-xl mx-auto">First send in under 5 minutes. Free warmup. Cancel any time.</p>
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Button size="lg" className="rounded-full h-12 px-7 bg-white text-primary hover:bg-white/90 font-bold" onClick={() => navigate({ to: "/login" })}>
                Get started free <ArrowRight className="w-4 h-4 ml-1" strokeWidth={2.5} />
              </Button>
              <Link to="/pricing">
                <Button size="lg" variant="outline" className="rounded-full h-12 px-7 bg-transparent border-white/40 text-white hover:bg-white/10 hover:text-white">
                  View pricing
                </Button>
              </Link>
            </div>
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-16 md:py-20 grid lg:grid-cols-2 gap-12 items-center">
      <div className={reverse ? "lg:order-2" : ""}>
        ...
        <h3 className="text-3xl md:text-4xl font-extrabold tracking-tight leading-tight">{title}</h3>
...
function LeadMagnet() {
  ...
  return (
    <section id="playbook" className="max-w-6xl mx-auto px-6 pb-24 scroll-mt-24">
      <div className="grid lg:grid-cols-5 gap-0 rounded-3xl border border-border bg-card overflow-hidden shadow-soft">
        {/* Left: copy + form */}
...
function FAQ() {
  const items = [
    ...
    { q: "How is pricing structured?", a: "Flat monthly plans based on email volume. Unlimited inboxes on every paid plan. No per-user, per-mailbox, or per-warmup fees. See the pricing page for details." },
    ...
    { q: "Is there a free trial?", a: "Yes — start free, no credit card required. The free tier includes one connected mailbox, free warmup, and full access to the AI Copilot." },
  ];
